...
def build_subagents(prompt_ctx: PromptContext | None = None) -> list[dict]:
    ...
    return [
        ...
        },
        {
            "name": "alice-codebook",
            "description": (
                "基于 codebook 样本的 Bob 结果归纳初始 constructs，写入 codebook/constructs.json。"
            ),
            "system_prompt": (
                ctx.alice_template
                + "\n\n你是子 Agent。从 coding/ 中读取 partition.codebook_text_ids 对应文件，"
                "归纳后写入 codebook/constructs.json。"
            ),
        },
        {
            "name": "kevin-updater",
            "description": (
                "在现有 codebook/constructs.json 基础上做保守增量更新，并写 batch 快照。"
            ),
            "system_prompt": (
                ctx.kevin_template
                + "\n\n你是子 Agent。读取 codebook/constructs.json 与 Kevin 批次对应 coding 文件，"
                "输出完整更新版 constructs。"
            ),
        },
    ]
